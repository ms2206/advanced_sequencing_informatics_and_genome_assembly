...
kmer=$(echo "$SOAP_ASSEMBELLY" | grep -oE 'kmer_[0-9]+' | cut -d_ -f2)


# Create output directory and change to it
output_dir="${WORKING_FOLDER}/assembly_qc/kat/${sample}_k${kmer}"
mkdir -p "${output_dir}"
cd "${output_dir}"

# Main code 
# ========================
echo "Running KAT comp for sample ${sample} with kmer size ${kmer}"
echo "Results will be saved to: ${output_dir}"

# Run KAT comp
singularity exec ${SINGULARITY} \
    kat comp \
    -o "${output_dir}/kat_comp_${sample}_k${kmer}" \
    -t 2 \

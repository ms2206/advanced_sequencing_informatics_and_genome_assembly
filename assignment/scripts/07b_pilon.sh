...
if [[ ! -e "genome.scf.fasta" ]]; then
#TODO: add final assembly as user input
    ln -s "/mnt/beegfs/home/s430452/advanced_sequencing_informatics_and_genome_assembly/assignment/assembly/masurca/HS7_R1.fastq.gz.cor_k64_soap_SR_masurca_hybrid_k64/CA.mr.41.15.17.0.029/9-terminator/genome.scf.fasta" .
fi

# Index the assembly
singularity exec ${SINGULARITY} \
    bwa index genome.scf.fasta

# Align the Illumina reads to the assembly
singularity exec ${SINGULARITY} \
    bash -c "\
    bwa mem -t ${cpus} genome.scf.fasta \
